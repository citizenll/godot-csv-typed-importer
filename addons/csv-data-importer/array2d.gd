...
func sort_row_custom(p_idx: int, p_func: Callable):
	_sort_axis_custom(p_idx, true, p_func)


func sort_col_custom(p_idx: int, p_func: Callable):
	_sort_axis_custom(p_idx, false, p_func)
...
func invert_row(p_idx: int):
	assert(p_idx >= 0 and len(data) > p_idx)
	data[p_idx].reverse()
	return self


func invert_col(p_idx: int):
	assert(len(data) > 0)
	assert(p_idx >= 0 and len(data[0]) > p_idx)
...
func _sort_axis_custom(p_idx: int, p_is_row: bool, p_func: Callable):
	if p_is_row:
		data[p_idx].sort_custom(p_func)
		return
	var col = get_col(p_idx)
	col.sort_custom(p_func)
	set_col(p_idx, col)
